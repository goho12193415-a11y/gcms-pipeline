...
def load_libraries(lib_dir=None):
    ...
    # NIST main library (generated by convert_nist_sdf.py)
    here = os.path.dirname(os.path.realpath(__file__))
    nist_path = os.path.join(here, 'nist_mainlib.msp')

    if not os.path.exists(nist_path):
        print("[LIB] ERROR: nist_mainlib.msp not found!")
        print("[LIB] Generate it: python convert_nist_sdf.py <mainlib.sdf> nist_mainlib.msp")
        print("[LIB] The NIST SDF is in: NIST_2014/Supplemental.zip (included with NIST MS Search)")
        sys.exit(1)

    comps = parse_msp(nist_path)
    all_compounds.extend(comps)
    print(f"[LIB] nist_mainlib.msp: {len(comps)} compounds")
